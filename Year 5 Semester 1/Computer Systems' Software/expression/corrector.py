    ...
    def correct(self) -> None:
        ...
        while self.i < len(self.expression)-1:
            ...
            if self.i == 0 and not (currentSymbol.isalnum() or currentSymbol in "-("):
                ...
                self.mishap(f"Невідомий символ '{currentSymbol}'", True)
                self.i -= 1
        ...
        if ''.join(self.expression) == self.initExpression:
            msg = Coloring.wrap('Вираз коректний!', Coloring.Color.Foreground.GREEN)
        else:
            msg = ("##### Кінцевий результат #####\n" +
                    Coloring.wrap(''.join(self.expression), Coloring.Color.Foreground.GREEN))
        print("-------------------------\n" +
              msg)
