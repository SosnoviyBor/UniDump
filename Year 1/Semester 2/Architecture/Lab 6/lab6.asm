...
DATASEG
message db "Team 1: Bukreev, Korol, Fedyay", 10, 13, "$"
...
Start:
	...
	; ----------------------------------------------------------------------------------
	; Призначення: Установка на номер вектора нової функції обробника. Зміна функціоналу функціоналу прерівання за логічною адресою
	; Вхід:
	...
	PROC SetIntVals
		...
		mov ds, ax				; DS <- 0
		shl	di, 2 				; DI <- DI * 4
		mov	[ds:[di]], dx 		; Перезаписуємо зміщення (IP) у векторі переривання 
		mov	[ds:[di + 2]], es 	; Перезаписуємо сегмент (CS) у векторі переривання 
